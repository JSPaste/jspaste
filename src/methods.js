...
async function remove(key, secret) {
    ...
    await a.delete(constants.baseURL + constants.routeURL + key, {headers: {secret: secret}})
        .catch(e => {
...
async function check(key) {
    ...
    const r = await a.get(constants.baseURL + constants.routeURL + key)
        .catch(() => false)
...
async function publish(body) {
    ...
    const r = await a.post(constants.baseURL + constants.routeURL, body)
        .catch(e => {
            return e.response ? handler.Error(request[0], request[1] + e.response.status + e.response.data.message ? e.response.data.message : e.message) : e
        })
